...
void Adafruit_MPR121::setThresholdH(uint8_t touch, uint8_t pin) {
  //for (uint8_t i=0; i<12; i++) {
    writeRegister(MPR121_TOUCHTH_0 + 2*pin, touch);
    //writeRegister(MPR121_RELEASETH_0 + 2*pin, release);
  //}
}

void Adafruit_MPR121::setThresholdL(uint8_t release, uint8_t pin) {
  //for (uint8_t i=0; i<12; i++) {
    //writeRegister(MPR121_TOUCHTH_0 + 2*pin, touch);
    writeRegister(MPR121_RELEASETH_0 + 2*pin, release);
  //}
